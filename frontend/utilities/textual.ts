function readableDate(term: Date | string, showYear: boolean = true, locale: string = "en-UK") {
  ...
  const day = readable.toLocaleDateString(locale, { day: "numeric" })
  const month = readable.toLocaleDateString(locale, { month: "short" })
  if (showYear) {
    const year = readable.toLocaleDateString(locale, { year: "numeric" })
