...
function setReview() {
    if (
        ...
        if (
            ...
            termOne.value = props.node.response.answer[0].value as string
            termTwo.value = props.node.response.answer[1].value as string
        } else if (
            !Array.isArray(props.node.response.answer)
            && props.node.response.answer.hasOwnProperty("0")
            && props.node.response.answer.hasOwnProperty("1")
        ) {
            // @ts-ignore
            termOne.value = props.node.response.answer["0"].value as string
            // @ts-ignore
