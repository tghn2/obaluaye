<template>
    <div class="sticky top-0 z-20 h-16 shrink-0 bg-white/75">
        <div class="border-b border-gray-200 pb-3">
            <div class="sticky top-0 z-20 flex shrink-0 items-center gap-x-4 sm:gap-x-6">
                <div class="flex w-full items-center justify-between gap-x-6 pb-2">
                    ...
                    <div class="flex flex-inline items-center space-x-2">
                        ...
                        <button type="button" @click.prevent="watchEditRequest('save')"
                            class="relative -ml-px inline-flex items-center gap-x-1.5 rounded-md px-3 py-2 text-sm text-gray-900 ring-1 ring-inset ring-gray-300 hover:bg-gray-50"
                            :disabled="groupStore.savingDraft">
                            <svg v-if="groupStore.savingDraft" aria-hidden="true"
                            ...
                            <span v-if="groupStore.savingDraft" class="hidden md:block">{{ t("header.saving") }}</span>
            ...
            <div v-if="journeyStore.term && journeyStore.term.hasOwnProperty('pathway')">
                <JourneyResponseBreadcrumbs 
                    :project="groupStore.term.pathway as ISummary" 
                    :group-id="groupStore.term.id as string"
                    :themes="groupStore.term.themes as ISummary[]"
                />
            </div>
        </div>
    </div>
...
import { PhHouseSimple, PhUpload, PhUsersThree, PhX } from "@phosphor-icons/vue"
import { useGroupStore, useJourneyStore } from "@/stores"
