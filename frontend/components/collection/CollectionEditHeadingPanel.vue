        ...
        <div class="flex w-full items-center justify-between gap-x-6 pb-2">
            <div class="flex items-center justify-left w-full space-x-4 truncate">
                <div class="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg bg-kashmir-500">
                    <PhRows class="h-5 w-5 text-white" aria-hidden="true" />
                </div>
            ...
            <div class="flex flex-inline items-center space-x-2">
                ...
                <CommonLocaleDropdown :language="collectionStore.draft.language as string"
                    @set-locale-select="watchLocaleSelect" :disabled="collectionStore.savingDraft" />
                <button type="button" @click.prevent="watchEditRequest('cancel')"
                    ...
                    :disabled="collectionStore.savingDraft">
                ...
                <button type="button" @click.prevent="watchEditRequest('save')"
                    class="relative -ml-px inline-flex items-center gap-x-1.5 rounded-md px-3 py-2 text-sm text-gray-900 ring-1 ring-inset ring-gray-300 hover:bg-gray-50"
                    :disabled="collectionStore.savingDraft">
                    <svg v-if="collectionStore.savingDraft" aria-hidden="true"
                    ...
                    <span v-if="collectionStore.savingDraft" class="hidden md:block">{{ t("header.saving") }}</span>
...
import { PhUpload, PhRows, PhX } from "@phosphor-icons/vue"
...
onMounted(async () => {
    if (!collectionStore.draft || !collectionStore.draft.language) collectionStore.setLanguageDraft(collectionStore.settings.locale)
})
</script>