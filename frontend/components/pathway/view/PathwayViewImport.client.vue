<template>
    <button @click.prevent="fileClickHandler" class="group flex flex-wrap justify-center mx-3 my-2 text-gray-900 hover:text-kashmir-600">
        <span v-if="!pathwayStore.savingDraft" class="hidden sm:block text-xs">{{ t("form.upload.upload") }}</span>
        <span v-else class="hidden sm:block text-xs">{{ t("pathway.journey.saving") }}</span>
        <svg v-if="pathwayStore.savingDraft" aria-hidden="true"
            class="h-5 w-4 text-kashmir-600 animate-spin fill-white" viewBox="0 0 100 101"
        ...
        <PhUploadSimple v-else class="h-5 w-4 shrink-0" aria-hidden="true" />
    </button>
</template>
...
async function fileClickHandler() {
    try {
        ...
        await pathwayStore.updateImportTerm(response)
    } catch (error: any) {
